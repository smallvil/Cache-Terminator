...
vcl_output_lang_h(struct vsb *sb)
{
	...
	vsb_cat(sb, "\n/* VCL Methods */\n#define VCL_MET_ACCEPT\t\t(1U << ");
	vsb_cat(sb, "0)\n#define VCL_MET_SOCKS_REQ\t(1U << 1)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_RECV\t(1U << 2)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_PIPE\t(1U << 3)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_PASS\t(1U << 4)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_HASH\t(1U << 5)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_MISS\t(1U << 6)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_HIT\t(1U << 7)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_FETCH\t(1U << 8)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_DELIVER\t(1U << 9)\n");
	vsb_cat(sb, "#define VCL_MET_HTTP_ERROR\t(1U << 10)\n");
	vsb_cat(sb, "\n#define VCL_MET_MAX\t\t11\n\n");
	vsb_cat(sb, "/* VCL Returns */\n#define VCL_RET_DELIVER\t\t0\n");
	vsb_cat(sb, "#define VCL_RET_DROP\t\t1\n#define VCL_RET_ERROR\t\t2\n");
	vsb_cat(sb, "#define VCL_RET_FETCH\t\t3\n#define VCL_RET_HASH\t\t4\n");
	vsb_cat(sb, "#define VCL_RET_HTTP\t\t5\n#define VCL_RET_LOOKUP\t\t6");
	vsb_cat(sb, "\n#define VCL_RET_PASS\t\t7\n#define VCL_RET_PIPE\t\t8");
	vsb_cat(sb, "\n#define VCL_RET_RESTART\t\t9\n");
	vsb_cat(sb, "#define VCL_RET_SOCKS\t\t10\n\n");
	vsb_cat(sb, "#define VCL_RET_MAX\t\t11\n\nstruct VCL_conf {\n");
	...
	vsb_cat(sb, "\tvcl_func_f\t*socks_req_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_recv_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_pipe_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_pass_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_hash_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_miss_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_hit_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_fetch_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_deliver_func;\n");
	vsb_cat(sb, "\tvcl_func_f\t*http_error_func;\n");
	vsb_cat(sb, "};\n");
	...
	vsb_cat(sb, " * SUCH DAMAGE.\n *\n * $Id: vrt.h 62 2011-04-08 17:37");
	vsb_cat(sb, ":18Z jwg286 $\n *\n * Runtime support for compiled VCL");
	...
	vsb_cat(sb, "ip(const struct sess *);\nconst char * VRT_r_client_id");
	vsb_cat(sb, "entity(struct sess *);\nvoid VRT_l_client_identity(str");
	vsb_cat(sb, "uct sess *, const char *, ...);\n");
	vsb_cat(sb, "const char * VRT_r_client_country(const struct sess *)");
	...
	vsb_cat(sb, "unsigned VRT_r_bereq_noflushreq(struct sess *);\n");
	vsb_cat(sb, "void VRT_l_bereq_noflushreq(struct sess *, unsigned);\n");
	...
	vsb_cat(sb, "eable(struct sess *);\nvoid VRT_l_beresp_cacheable(str");
	vsb_cat(sb, "uct sess *, unsigned);\ndouble VRT_r_beresp_ttl(struct");
	vsb_cat(sb, " sess *);\nvoid VRT_l_beresp_ttl(struct sess *, double");
	vsb_cat(sb, ");\ndouble VRT_r_beresp_grace(struct sess *);\n");
	vsb_cat(sb, "void VRT_l_beresp_grace(struct sess *, double);\n");
	...
	vsb_cat(sb, "char *, ...);\nconst char * VRT_r_socks_type(const str");
	vsb_cat(sb, "uct sess *);\nstruct sockaddr * VRT_r_socks_ip(struct ");
	vsb_cat(sb, "sess *);\nstruct director * VRT_r_socks_backend(struct");
	vsb_cat(sb, " sess *);\nvoid VRT_l_socks_backend(struct sess *, str");
	vsb_cat(sb, "uct director *);\ndouble VRT_r_now(const struct sess *");
	vsb_cat(sb, ");\n");
}
